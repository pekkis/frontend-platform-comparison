...
export const getHeadlines = async (limit: number | undefined = undefined) => {
	const headlines = await graphQLClient.request<HeadlinesQueryResponse>(headlinesQuery, {
		limit
	});
...
export const getBlogPosts = async (slug: string) => {
	const ret = await graphQLClient.request<BlogPostQueryResponse>(blogPostQuery, {
		slug
	});
